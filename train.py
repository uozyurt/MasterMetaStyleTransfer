...
class Train:
    ...
    def clone_as_leaves(self, named_parameters):
        return {name: torch.nn.Parameter(param.detach()) for name, param in named_parameters}
    ...
    def train(self):
        ...
        for iteration in tqdm(range(1, self.max_iterations + 1)):
            ...
            """            # Copy the model parameters to omega
            omega = {name: param.clone() for name, param in self.style_transformer.named_parameters()}
            if not self.freeze_encoder:
                omega.update({name: param.clone() for name, param in self.swin_encoder.named_parameters()})
            omega.update({name: param.clone() for name, param in self.decoder.named_parameters()})
            
            # Adam optimizer for omega
            omega_params = [param for param in omega.values()]
            optimizer = optim.Adam(omega_params, lr=self.inner_lr)"""

            # Clone parameters as leaves
            omega = self.clone_as_leaves(self.style_transformer.named_parameters())
            if not self.freeze_encoder:
                omega.update(self.clone_as_leaves(self.swin_encoder.named_parameters()))
            omega.update(self.clone_as_leaves(self.decoder.named_parameters()))

            # Extract the leaf tensors
            omega_params = list(omega.values())

            # Create an optimizer for omega
            optimizer = optim.Adam(omega_params, lr=self.inner_lr)
            ...
            for _ in range(self.num_inner_updates):
                ...
                # style transfer
                transformed_output = self.style_transformer(encoded_content, encoded_style, num_layers)

                transformed_output = transformed_output.permute(0, 3, 1, 2).to(self.device)
                # Decode the transformed output
                decoded_output = self.decoder(transformed_output)
            ...
            for name, param in self.style_transformer.named_parameters():
                param.data += self.outer_lr * (omega[name] - param)
            if not self.freeze_encoder:
